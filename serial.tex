...
In this section, the dataset consists of 11637 frames of Eiger 4M images, out of which 11512 were properly indexed. 
Those data were collected on the ID30A3 beamline at the ESRF (Massif3) from tiny crystals of egg white lysozyme (HEWL) + Gadolinium, deposited on a SiN membrane and this membrane was scanned in front of the beam.
...
\end{center}
\end{figure}
As expected, the crystallographic quality indicator show a gradual degradation with increasingly sparse data but Rfree shows little degradation with little sparsification ($0.8\sigma$ equivalent to 21\% of the pixels saved).

\subsubsection{Quality indicators as function of the resolution shell:}
The same Rfree/Rwork quality indicators were calculated for the initial dataset for different resolution shells to ensure the degradation is not only limited in the outer-shell. 
Those indicators are compared in the figure \ref{nicolas_R} with the one obtained from the same dataset sparsified at $0.8\sigma$ and $2.0\sigma$ and subsequently re-densified.
...
Both Rfree and Rwork exhibit degradation but not necessary worse within the outer-shells. 
...
\subsection{Influence of the sparsification on the quality of serial-crystallography data acquired with Jungfrau detector}
Small crystals of NQO1 sample \cite{NQO1}, complexed with NADH, were collected at ESRF-ID29 using a fixed-target sample environment and the jungfrau 4M detector. 
...
The indexation was performed with the \textit{xgandalf} algorithm \cite{xgandalf} with default settings from CrystFEL 0.10.1 and was provided with the cell parameter: tetragonal $a=b=78.77\text{\AA}; c=39.04\text{\AA}; \alpha=\beta=\gamma = 90^o$.
...
This means the peak positions provided by the veto-algorithm are suitable for indexing and this save the reading of the complete frame if it cannot be indexed.
...
With data from the Jungfrau detector, peak positions are systematically off and the indexing rate are much lower than with the peak-finder integrated into CrystFEL.
...
A first application presented is a lossy compression of diffraction frames, called sparsification, where the average background is saved on the one side and the position and the intensity of most intense pixels (probably belonging to peaks) on the the other.
The quality of the compression has been demonstrated on macro-molecular rotational and serial crystallography data. 
The degradation of the signal has been monitored after a cycle of compression-decompression, both in reciprocal space using Rfree/Rwork crystallographic indicators and after Fourier transform in direct space where the number of residues placed phasing was checked and the degradatoi  . .
...
One of the strength of this peak-finer algorithm is that it is optimized to work on GPU and thus ideally suited to be coupled with the next generation of crystal indexer \cite{toro} which is running on the same kind of hardware, allowing to save the memory-transfer time and maybe achieve real-time integration of serial crystallography data.
