...
While shadows can be handled with a user defined mask, anisotropy in the background scattering, as it is sometimes observed with certain stretched plastic films in fixed-target mode, is much more challenging and will not be addressable wit this analysis.   
...
\begin{itemize}
    \item Pixel intensities and positions for pixels worth saving.
...
\begin{itemize}
    ...
    \item  Use the burst mode: acquire only shorter datasets and make breaks to let the data be saved from the detector computer to the central storage.
    \item Discard pixel of lower intensity: this is a new possibility offered by this algorithm. A cutoff at $1.4\sigma$ is already providing the 6.4x compression ratio needed according to (cf. figure \ref{distribution}).
\end{itemize}
...
The main bottleneck remains the networked saving of the different files: raw files, sparse files, peak positions, accumulated frames, ... not all can be saved at all time when operating at full speed.
...
Tiny crystals of egg white lysozyme (HEWL) + Gadolinium were deposited on a SiN membrane and this membrane was scanned in front of the beam at the ID30A3 beamline at the ESRF (Massif3) with an Eiger 4M detector.
This dataset consists of 11637 frames out of which 11512 were indexed.

\subsubsection{Quality indicator degradation:}
The Rfree/Rwork quality indicators \cite{Rfree} were calculated for the initial dataset and compared with the same dataset sparsified at 0.8, 1.0, 1.4 and $2.0\sigma$ in the figure \ref{}

\begin{figure}
\label{Rfree}
\begin{center}
\includegraphics[width=9cm]{figure_rfree}
...
\subsubsection{Quality indicator degradation as function of the resolution shell:}
The Rfree/Rwork quality indicators \cite{Rfree} were calculated for the initial dataset for different resolution shells. 
Those indicatores are compared in the figure \ref{nicolas_R} with the one obtained from the same dataset sparsified at $0.8\sigma$ and $2.0\sigma$ and subsequently re-densified.
...
Both Rfree and Rwork exhibit degradation of the 
...
[...]
I looked at the possibility or not to phase, and the difference in R/Rfree between original and sparsified (then reconstructed) data.

Regarding the comment on weakly diffracting crystals of the reviewers, one way to "artificially" decrease the anomalous signal strength is to lower the number of indexed patterns in a dataset.
The original dataset contains ~11k indexed patterns. We could phase with as low as 3.5k indexed patterns (both original and sparsified).
...
Please see the attached document, which briefly present a part of the analysis I did. 
...
\subsection{Performances on a serial crystallography dataset}
Small crystals of NQO1 sample \cite{NQO1}, complexed with NADH, were collected at ESRF-ID29 using a fixed-target sample environment. 
...
The indexation rate for dense data, and two sparse dataset (cut-off at 1.0$\sigma$ and 1.4$\sigma$) are reported in table \ref{NQO1}. 
...
There is little on can say, except the number of indexed frames drops slightly.

TODO: place here the analysis of the electron density
...
This excludes notably diffuse scattering experiment but it is generally applicable to most single crystal diffraction experiments, ranging from small molecules to macro-molecules like proteins and even serial crystallography.
With the addition compression offered, sparsification is especially interesting for serial crystallography with datasets of millions of frames collected per experiment.
As any lossy compression, the user has still the responsibility of choosing the level under which all data will be lost, which will limit later-on the quality of the results one may extract from those data. 
...
In comparison with ROIBIN-SZ \cite{roibin}, which preserves pixels in the neighborhood of identified peaks and stores the background heavily binned, the sparsification presented here does not require a complete peak-search thus it is simpler, with fewer parameters to be tuned, and is able to save any pixel which is intense enough.
...
The sigma clipping algorithm provides us with the background (average and deviation) and is used to pick pixels which are likely to be Bragg peaks, like peakfinder8 \cite{Cheetah2014} does. 
...
\item Enough of pixels in the local neighborhood satisfying the SNR condition: typically 2 to 5 pixels out of $3\times 3$ or $5\times 5$ are intense enough (parameters set manually).
\end{itemize}
...
\label{quality-pf}
The quality of peaks extracted with this algorithm is finally evaluated on a serial-crystallography dataset: tiny crystals of egg white lysozyme (HEWL) were deposited on a SiN membrane and this membrane was scanned in front of the beam at the ID30A3 beamline at the ESRF with an Eiger 4M detector.
A fraction of 1000 frames of the dataset %\cite{ssx-Lyso} 
used in section \ref{nicolas} 
...
TODO: Comparison with Eiger detector ...

Disclaimer about the sparsification algorithm
