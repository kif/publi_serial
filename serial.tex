...
\item Enough of pixels in the local neighborhood satisfying the signal over noise ratio (SNR) condition: $|I-\bar{I} |> n \sigma_{I}$, typically 2 to 5 pixels
\end{itemize}
...
\caption{Tree structure (left-hand side) and default plot with the number of peaks found per frame (right-hand side) of a CXI file produced by pyFAI and visualized with the silx viewer \cite{silx}.}
\end{figure}


the data and validating if those peaks correspond to any realistic crystal lattice \cite{nanopeakcell}.
...
Figure \ref{peakfinder} presents the comparison between the original \textit{peakfinder8} described in \citeasnoun{Cheetah2014}, interfaced in Python via OnDA \cite{onda} and the version implemented into pyFAI on the same Pilatus 6M image used in figure \ref{fig1}. 
%More advanced peakfinder algorithm are available in CrystFEL like \textit{peakfinder9} but it is not available as 

\begin{figure}
\label{peakfinder}
\includegraphics[width=12cm]{peakfinder}
\caption{Comparison of the reference \textit{peakfinder8} interfaced with Onda (in orange, execution time 300ms) and the version from pyFAI (in light blue, execution time 10ms) on top of a Pilatus6M protein diffraction frame.}
\end{figure}

In this figure \ref{peakfinder}, most peak found by both implementation correspond and superimpose with Bragg-peak visible by a crystallographer's eye.
One may have the impression that more blue-peaks are found by pyFAI close to the beam center while more orange peaks are spotted by Onda in the outer-shell.
This plot was made with a minimum SNR of three and a noise level of one since the Pilatus detector is Poissonian.
Peaks were registered if four pixels meet the SNR criterion in a 5x5 pixel patch around the peak.
...
This is likely due to the curvature of the Debye-Scherrer ring: the original version of peakfinder8 evaluates the variance in a neighborhood defined by some radius around the point of interest and the variance is higher there due to the important curvature closer to the beam-center.
On the opposite, pyFAI knows about this curvature and measures the variance on the complete ring.
...
Interestingly, an entry level GPU (Nvidia GT1030), which is seven times cheaper than Epyc processor, has the the same performances (but the CPU has eight cores). 
...
The indexation was performed with the \textsc{xgandalf} algorithm with default settings from CrystFEL 0.10.1 and was provided the cell parameter: tetragonal Lysozyme $a=b=78.77\AA; c=39.04\AA; \alpha=\beta=\gamma = 90°$.
The table \ref{crystfel} compares the number of frames properly indexed with the different picking algorithms available in CrystFEL with the algorithm presented. 

\begin{table}
\label{crystfel}
\caption{Quality of the indexing with various peak-picker available in CrystFEL\cite{CrystFEL}}.
\begin{tabular}{|c| c | c |} 
\hline
Method & indexation rate & run-time (s) \\ 
\hline
Zaef \cite{zaefferer} & 16\textperthousand & 572s \\
PeakFinder8 \cite{Cheetah2014} & 456\textperthousand& 1577s?\\
PeakFinder9 \cite{CrystFEL} & 442\textperthousand& 8307s?\\
pyFAI (this contribution) & 481\textperthousand& 8307s?\\
\hline
\end{tabular}
\end{table}

The \textit{robust peak-finder} algorithm is not yet available from within CrystFEL and the source code provided at \citeasnoun{robustpeakfinder} does not compile, thus its performance was not compared.
The performances obtained with the algorithm described in this contribution is slightly better than reference implementations like \textit{peakfinder8} or \textit{peakfinder9}. 
The execution time for peak extraction is similar to the one used for the sparsification: 
this represents less than 30s for 1000 frames but most of the time is spent in reading and writing HDF5 files.
