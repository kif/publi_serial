...
Since the background extraction and peak finding is performed in real-time on the serial-crystallography beamline ID29 at ESRF, the information about the number of Bragg-spots can be used to assess the quality of each individual image and the acquisition system can decide to discard the frame depending on the number of peaks and a live-adjustable threshold. 

For a couple of years, the beamline operated with the $veto$ algorithm deactivated, the result of the processing was just used to create a flag "hit" or "no-hit" depending if the number of peaks. 
The indexation rate of "hit" and "no-hit" frames acquires on XXXX microcrystals are summarized in the table \ref{veto-table}:
...
Since the Jungfrau is an integrating detector, the detector has relatively more background noise than a photon counting detector.
Thus the compression rate of hit and non-hit frames is similar and discarding ???\% non-hit frames of would have allowed to save as much network bandwidth and disk space.
The $veto$ algorithm having proved its robustness, it is activated for most experiments since 2024.


\subsection{Limitations}
The Jungfrau 4M detector is built from 8 modules manually assembled which have some remaining misalignment. 
The geometry of this detector used for the azimuthal integration is based on pyFAI and differs from the geometry used by the indexing software. 
We noticed a very strong sensitivity of the indexation rate on the peak-position (<1 pixel) with data acquired with the Jungfrau detector on ID29 (unlike the experiment on Eiger described in \ref{quality-pf}), which could be linked to the detector geometric distortion and/or the pink beam. 
