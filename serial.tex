...
The maximum theoretical compression ratio for $2\sigma$ is 22x (from normal distribution: $1/(1-erf(\sigma/\sqrt{2})$), neglecting the storage of the background data and effects of the lossless compression, intensity and position needs to be stored for each pixel).
To evaluate the effective maximal compression ratio, the dataset was median-filtered along the image stack
to produce an image without any peaks. 
A background dataset of 1800 of those images sparsifies into a 11MB HDF5 file which represents a compression ratio of 136x ! 
Indeed, only 19 pixels were saved per frame and the compressed numerical values are mostly the same, which compress great.
...
For the previous example (ID29) where a Jungfrau 4M operating at 1kHz, which frames needs some `pedestal` preprocessing converting uint16 integers into float32 data types, thus doubling the needed bandwidth. 
The detector outputs the data via 8x 10 Gbit/s network links and we need to saved then all with a single  25 Gbit/s link, making a minimum compression ratio of 6.4x or an equivalent cutoff of $1.4\sigma$.
