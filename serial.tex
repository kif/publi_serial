...
In macro-molecular crystallography (MX), several crystals are often need to collect a complete dataset.
Serial crystallography overcomes radiation damage by exposing each crystal a single time, thus, hundreds or thousands of them are needed to collect a complete dataset, and exposed in a serial way.
...
The technique was first developed for X-ray free electron lasers (\textsc{xfel}) \cite{Chapman2011} and then adapted to be performed at synchrotron light sources \cite{ssx, ssx_desy, ssx_id13, ssx_diamond}.
...
With the absence of read-out noise and their fast speed, their only limitation is the achievable count-rate, i.e. the number of photons a pixel is able to count during the exposure.
% difference bewteen pixel counting vs integrating, in speed ...
Unlike photon-counting detectors such as the Eiger detector \cite{Eiger}, the Jungfrau detector \cite{jungfrau2016} is an integrating detector thus, it is not limited by the count rate, even under very intense flux as expected when recording Bragg-peaks.
...
Since the Jungfrau detector is an integrating detector, dark current and flat-field correction have to be applied: every pixel has three dark-current values (called pedestals) and three gain values (one for each gain level). 
...
Thus efficient processing of raw images is vital for SSX, with 4 levels of increasing complexity:
\begin{itemize}
    \item image reconstruction with pedestal correction
    \item \textit{veto-}algorithm able to sieve out images with poor signal
...
The simplest implementation of Bragg peaks separation is to assume that the background originates from scattering from amorphous material or from an isotropic powder, both giving isotropic signal ideally presenting only smooth variations.



For this, the raw signal has to be corrected from dark noise and any systematic anisotropic effects such as polarization corrections.
Unlike X-FEL, synchrotron X-ray beam is well characterized in energy and shows little bunch-to-bunch variability. 
All anisotropic correction can be easily taken into account.
...
Nevertheless this method has been successfully used for large dataset analysis \cite{brocades}, it presents four major drawbacks:
\begin{itemize}
...
\item The median filter is computationally heavy since it requires to sort out every azimuthal bin.
\end{itemize}
...
In the case of the Jungfrau detector considered for the ESRF-ID29 beam-line, this integrating detector has a complex gain switching mechanism \cite{jungfrau_PSI} which makes this equation complicated.
...
\citeasnoun{variance2018} present the way to performing the union of two sub-partitions $A$ and $B$ of a larger ensemble and opens the doors to parallel reductions, which are especially efficient when implemented on GPU:
\begin{equation}
...
This clipping re-centers the distribution of remaining pixels and enforces a normal distribution of the remaining values:
\begin{equation}
\label{clip}
|I - <I>| > n \cdot \sigma(I)
\end{equation}
...
Lysozyme sample provided by Dectris for advertizing their Eiger 4M detector (TODO).}.
...
Since the \textit{robust peak-finder} algorithm is not yet available from within CrystFEL and the source code provided in \citeasnoun{robustpeakfinder} does not compile, its performance was not compared to the others.
