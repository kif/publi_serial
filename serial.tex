...
This document will focus on the subsequent steps: peak-finding is addressed in section 2, peak location and indexing in section 3, leaving real-time integration as a challenge for the future.
...
If the initial distribution was mono-modal, this algorithm enforces gradually the data to be sampled symmetrically around the maximum probability which is likely to look like a normal distribution.
If the initial distribution was more complicated (typically multi-modal), the necessary larger standard-deviation will prevent most outlier pixels from being rejected, making it conservative in complicated cases.
