...
Since the Jungfrau detector is an integrating detector, dark current and flat-field correction have to be applied: every pixel has three called pedestals and three gain values (one for each gain level). 
...
The simplest implementation of Bragg peaks separation is to assume that the background signal originates from scattering from amorphous material or from an isotropic powder, both giving an isotropic signal that ideally presents only smooth variations.
...
This statistical law states that the variance for a pixel is the number of events counted (in average).
...
\citeasnoun{variance2018} present a complete review, which introduces a formalism adapted here for crystallography.
